...
const BookingModal = ({ event, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const available = seatData[preference].available || 0;
    const payingMembers = Math.max(formData.adults - formData.children, 0);
    ...
    try {
      ...
      // ✅ Update only available seat count
      await update(seatRef, {
        [preference]: {
          ...seatData[preference],
          available: available - payingMembers,
        },
      });
      ...
      <div className="modal booking-modal">
        ...
        {loading ? (
          ...
          <form onSubmit={handleSubmit}>
            ...
            <label>Children (Below 5 - Free)</label>
            <input type="number" name="children" min="0" max={formData.adults} value={formData.children} onChange={handleChange} />
